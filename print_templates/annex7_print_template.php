<?php
/**
 * NDRRMC Annex 7: Damage to Other Assets - Print Template
...
<head>
    <meta charset="UTF-8">
    <title>NDRRMC Annex 7 - Damage to Other Assets</title>
    <style>
        @page {
...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Particulars /<br>Name of Asset</th>
                <th>Unit</th>
                <th>Quantity</th>
                <th>Cost</th>
                <th>Remarks</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-left"><?php echo htmlspecialchars($record['particulars'] ?: ''); ?></td>
    ...
    <div style="margin-top: 25px; font-size: 9pt; line-height: 1.6;">
        ...
        <?php
        $total_quantity = array_sum(array_column($records, 'quantity'));

        // Count by classification
        $classification_counts = [];
        foreach ($records as $record) {
            $classification = $record['classification'] ?? 'Other';
            $classification_counts[$classification] = ($classification_counts[$classification] ?? 0) + 1;
        }
        ?>
        Total Assets Damaged: <strong><?php echo count($records); ?></strong> &nbsp;|&nbsp;
        Total Quantity: <strong><?php echo number_format($total_quantity, 2); ?></strong><br>
        <strong>By Classification:</strong><br>
        <?php foreach ($classification_counts as $classification => $count): ?>
        <?php echo htmlspecialchars($classification); ?>: <strong><?php echo $count; ?></strong> &nbsp;|&nbsp;
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
