...
define('PAGE_TITLE', 'Annex 15 - Archived Suspension of Classes Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex15_archived_access',
    'Accessed Annex 15 archived records page'
);
// Database connection
$pdo = $pdo;
...
try {
    ...
    if ($column_check->rowCount() == 0) {
        $pdo->exec("ALTER TABLE annex15_suspension_classes ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex15 archived CSRF token mismatch');
        header('Location: annex15_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex15_suspension_classes WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex15 record #{$restore_id} without permission");
                header('Location: annex15_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex15_suspension_classes WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex15_suspension_classes SET is_archived = 0 WHERE id = ?", [$restore_id]);
            
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex15_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}: {$record['level']} - {$record['type']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex15 record #{$delete_id} without permission");
                header('Location: annex15_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex15_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}: {$record['level']} - {$record['type']}"
                );
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 15: Archived Suspension of Classes Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex15_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="10">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived suspension of classes reports.</p>
                                    <a href="annex15_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Level,Type,Suspension Date,Resumption Date,Suspension Reason,Issuing Authority,Suspension Scope,Alternative Delivery,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex15_archived_" + new Date().toISOString().split('T')[0] + ".csv");
