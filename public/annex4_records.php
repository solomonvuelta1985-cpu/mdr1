...
define('PAGE_TITLE', 'Annex 4 - Damaged Houses Records');
...
if (!check_rate_limit($user_id, 'annex4_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex4 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex4_records_access',
    'Accessed Annex 4 damaged houses records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex4_damaged_houses LIKE 'is_archived'");
    ...
    if ($column_check->rowCount() == 0) {
        // SECURITY FIX - DO NOT AUTO-ALTER: // $pdo->exec("ALTER TABLE annex4_damaged_houses ADD COLUMN is_archived TINYINT(1) DEFAULT 0");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex4 records CSRF token mismatch');
        header('Location: annex4_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex4_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex4 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex4');
            header('Location: annex4_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex4_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex4 delete rate limit exceeded');
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex4_damaged_houses WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex4 record #{$delete_id} without permission");
                header('Location: annex4_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex4_damaged_houses WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("DELETE FROM annex4_damaged_houses WHERE id = ?", [$delete_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex4_delete_success',
                    "Successfully deleted Annex4 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex4 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex4');
            header('Location: annex4_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex4_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex4 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex4_damaged_houses WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex4 record #{$archive_id} without permission");
                header('Location: annex4_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex4_damaged_houses WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex4_damaged_houses SET is_archived = 1 WHERE id = ?", [$archive_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex4_archive_success',
                    "Successfully archived Annex4 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex4 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
foreach ($records as $record) {
    ...
    $total_houses += $record['total_damaged'] ?? 0;
}
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-house-damage"></i> Damaged Houses Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 4</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            <h3><?php echo number_format($total_totally_damaged); ?></h3>
            <p><i class="fas fa-house-crack"></i> Totally Damaged</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_partially_damaged); ?></h3>
            <p><i class="fas fa-house-signal"></i> Partially Damaged</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_houses); ?></h3>
            <p><i class="fas fa-houses"></i> Total Damaged</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex4.php" class="btn-custom btn-primary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            ...
                            <p style="color: #6c757d; margin-top: 15px;">No damaged houses records found.</p>
                            <a href="annex4.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                ...
                <?php else: ?>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-totally"><?php echo $record['totally_damaged']; ?></span>
                            </td>
                            <td>
                                <span class="badge badge-partially"><?php echo $record['partially_damaged']; ?></span>
                            </td>
                            <td>
                                <span class="badge badge-total"><?php echo $record['total_damaged']; ?></span>
                            </td>
                            <td>
                                <?php if (!empty($record['cost'])): ?>
                                    <?php echo htmlspecialchars($record['cost']); ?>
                                <?php else: ?>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Damaged Houses Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Damaged Houses Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex4_view.php?id=${recordId}`)
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex4_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    // Headers
    csvContent += "ID,Barangay,Totally Damaged,Partially Damaged,Total Damaged,Cost,Remarks,Created By,Created At\n";
    ...
    link.setAttribute("download", "annex4_damaged_houses_" + new Date().toISOString().split('T')[0] + ".csv");
