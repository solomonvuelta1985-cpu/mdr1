...
define('PAGE_TITLE', 'Annex 6 - Archived Infrastructure Damage Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex6_archived_access',
    'Accessed Annex 6 archived records page'
);
// Database connection
$pdo = $pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        header('Location: annex6_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            ...
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }

        if ($record) {
            $stmt = db_query("UPDATE annex6_infrastructure_damage SET is_archived = 0 WHERE id = ?", [$restore_id]);
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 6: Archived Infrastructure Damage Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex6_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived infrastructure damage reports.</p>
                                    <a href="annex6_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
                    ...
                    <?php else: ?>
                        <?php foreach ($archived_records as $record): ?>
                            <tr>
                                ...
                                <td><strong><?php echo htmlspecialchars($record['name'] ?: 'Not specified'); ?></strong></td>
                                <td><?php echo number_format($record['total_damaged']); ?> units</td>
                                <td>
                                    <small>Unit: <?php echo htmlspecialchars($record['unit'] ?: 'N/A'); ?></small><br>
                                    <small>Qty: <?php echo number_format($record['quantity'], 2); ?></small>
                                </td>
                                <td>
...
function exportToCSV() {
    ...
    link.setAttribute("download", "annex6_archived_" + new Date().toISOString().split('T')[0] + ".csv");
