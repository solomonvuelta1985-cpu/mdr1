...
define('PAGE_TITLE', 'Annex 7 - Damage to Other Assets');
...
log_audit_action($_SESSION['user_id'], 'annex7_form_access', 'User accessed the Annex 7 Other Assets Damage form');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'])) {
        ...
        header('Location: annex7.php');
        exit;
    }

    // Rate limiting - 10 submissions per 5 minutes
    if (!check_rate_limit($_SESSION['user_id'], 'annex7_submission', 10, 300)) {
        ...
        try {
            ...
            foreach ($_POST['region'] as $index => $region) {
                ...
                $entry_data = [
                    ...
                    'classification' => sanitize($_POST['type'][$index] ?? ''),  // type field maps to classification in DB
                    'particulars' => sanitize($_POST['name'][$index] ?? ''),      // name field maps to particulars in DB
                ...
                if (empty($entry_data['region']) || empty($entry_data['province']) ||
                    ...
                    empty($entry_data['classification'])) {
                ...
                if (!in_array($entry_data['classification'], $valid_classifications)) {
                    $validation_errors[] = "Entry " . ($index + 1) . ": Invalid asset classification";
                    $error_count++;
                    continue;
                }
                ...
                if (strlen($entry_data['particulars']) > 500) {
                    $validation_errors[] = "Entry " . ($index + 1) . ": Asset particulars too long (max 500 characters)";
                    $error_count++;
                    continue;
                }
                ...
                $stmt = $pdo->prepare("
                    ...
                    (region, province, city, barangay, classification, particulars,
                     unit, quantity, cost, remarks, created_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                if ($result) {
                    ...
                    log_audit_action($_SESSION['user_id'], 'annex7_submission',
                        "Submitted other assets damage report for " . $entry_data['barangay'] .
                        ": " . $entry_data['classification'] . " - " . $entry_data['particulars']);
                } else {
                    $error_count++;
                }
            }
    ...
    // Set flash messages
    if ($success_count > 0) {
        set_flash("Successfully saved $success_count other assets damage report(s).", 'success');
    }
    if ($error_count > 0) {
    ...
    <div class="form-container">
        ...
        <div id="formContent" class="form-content">
            <div class="header-section">
                ...
                <h2>Annex 7: Damage to Other Assets</h2>
            ...
            <form id="infrastructureDamageForm" method="POST">
                ...
                <div id="entriesContainer">
                    ...
                    <div class="entry-card lazy-entry loaded">
                        ...
                        <div class="section-title mt-4">Asset Details</div>
                        <div class="row g-3">
                            <div class="col-md-6 col-lg-4">
                                <label for="type-0" class="form-label">Classification</label>
                                <select class="form-select" id="type-0" name="type[]" required>
                                    <option value="">Select Classification</option>
                                    <option value="Vehicles">Vehicles</option>
                                    <option value="Aircraft">Aircraft</option>
                                    <option value="Weapons / Ammunition">Weapons / Ammunition</option>
                                    ...
                                    <option value="Electronics / ICT Equipment">Electronics / ICT Equipment</option>
                                    <option value="Other">Other</option>
                                </select>
                            </div>
                            <div class="col-md-6 col-lg-8">
                                <label for="name-0" class="form-label">Particulars / Name of Asset</label>
                                <input type="text" class="form-control" id="name-0" name="name[]" placeholder="Enter asset name/description" maxlength="500">
                            </div>
                        </div>

                        <div class="section-title mt-4">Quantity and Cost Details</div>
                        <div class="row g-3">
                            <div class="col-md-4 col-lg-3">
                                <label for="unit-0" class="form-label">Unit</label>
                                <input type="text" class="form-control" id="unit-0" name="unit[]" placeholder="e.g., piece, set, unit" maxlength="50">
                            </div>
                            <div class="col-md-4 col-lg-3">
                            ...
                            <div class="col-md-4 col-lg-6">
                                <label for="cost-0" class="form-label">Estimated Cost of Damage</label>
                                <input type="text" class="form-control" id="cost-0" name="cost[]" placeholder="Enter estimated cost" maxlength="100">
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            Specify the exact geographic location where the other assets damage occurred.</p>
                            ...
                            <p><strong>Particulars / Name of Asset</strong><br>
                        ...
                        <div class="col-md-6">
                            ...
                            Indicate the total estimated cost of other assets damage, if available.</p>
            ...
            newEntry.innerHTML = `
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6 col-lg-6">
                        <label for="name-${entryCount}" class="form-label">Particulars / Name of Asset</label>
                        <input type="text" class="form-control" id="name-${entryCount}" name="name[]" placeholder="Enter asset name/description" maxlength="255">
                    </div>
                </div>
