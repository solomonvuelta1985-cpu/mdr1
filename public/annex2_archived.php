...
define('PAGE_TITLE', 'Annex 2 - Archived Affected Population Records');
...
if (!check_rate_limit($user_id, 'annex2_archived_access', 80, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex2 archived page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex2_archived_access',
    'Accessed Annex 2 archived affected population records page'
);
...
log_security_event(
    $_SESSION['user_id'],
    'page_access',
    'Accessed Annex 2 archived records page'
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex2 archived CSRF token mismatch');
        header('Location: annex2_archived.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex2_archived_action', 20, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex2 archived POST action rate limit exceeded');
    ...
    if (isset($_POST['restore_id'])) {
        $restore_id = filter_var($_POST['restore_id'], FILTER_VALIDATE_INT);
        
        if (!$restore_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid restore_id for Annex2');
            header('Location: annex2_archived.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for restore operations
        if (!check_rate_limit($user_id, 'annex2_restore', 15, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex2 restore rate limit exceeded');
            set_flash('Too many restore attempts. Please try again later.', 'error');
            header('Location: annex2_archived.php');
            exit;
        }
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex2_affected_population WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex2 record #{$restore_id} without permission");
                header('Location: annex2_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex2_affected_population WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex2_affected_population SET is_archived = 0 WHERE id = ?", [$restore_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex2_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}"
                );
                
                // SECURITY LOG: Successful restoration
                log_security_event(
                    $_SESSION['user_id'],
                    'annex2_restore_success',
                    "Successfully restored Annex2 record #{$restore_id}"
                );
                
                set_flash('Record restored successfully', 'success');
            } else {
                set_flash('Failed to restore record', 'error');
                log_security_event($_SESSION['user_id'], 'restore_failure', "Failed to restore Annex2 record #{$restore_id}");
            }
        } else {
    ...
    if (isset($_POST['permanent_delete_id'])) {
        $delete_id = filter_var($_POST['permanent_delete_id'], FILTER_VALIDATE_INT);
        
        if (!$delete_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid permanent_delete_id for Annex2');
            header('Location: annex2_archived.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for permanent delete operations
        if (!check_rate_limit($user_id, 'annex2_permanent_delete', 5, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex2 permanent delete rate limit exceeded');
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex2 record #{$delete_id} without permission");
                header('Location: annex2_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex2_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}"
                );
                
                // SECURITY LOG: Successful permanent deletion
                log_security_event(
                    $_SESSION['user_id'],
                    'annex2_permanent_delete_success',
                    "Successfully permanently deleted Annex2 record #{$delete_id}"
                );
                
                set_flash('Record permanently deleted', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to permanently delete Annex2 record #{$delete_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-archive"></i> Archived Affected Population Records</h1>
        <p>View and restore archived affected population reports (Annex 2)</p>
    </div>
    ...
    <div class="action-section">
        <div class="search-box">
            <input type="text" id="searchInput" placeholder="🔍 Search archived records..." onkeyup="searchRecords()">
        </div>
        <div class="action-buttons">
            <a href="annex2_records.php" class="btn-custom btn-primary-custom">
                ...
                <?php if (empty($archived_records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '11' : '10'; ?>" style="text-align: center; padding: 30px;">
                            ...
                            <p style="color: #6c757d; margin-top: 15px;">No archived records found.</p>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($archived_records as $record): ?>
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-cumulative">
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Affected Families (Cumulative),Affected Persons (Cumulative),Total Families (Cumulative),Total Persons (Cumulative),Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex2_archived_" + new Date().toISOString().split('T')[0] + ".csv");
