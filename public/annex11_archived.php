...
define('PAGE_TITLE', 'Annex 11 - Archived Communication Lines Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex11_archived_access',
    'Accessed Annex 11 archived records page'
);
...
try {
    ...
    if ($column_check->rowCount() == 0) {
        $pdo->exec("ALTER TABLE annex11_communication_lines ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex11 archived CSRF token mismatch');
        header('Location: annex11_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex11_communication_lines WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex11 record #{$restore_id} without permission");
                header('Location: annex11_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex11_communication_lines WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex11_communication_lines SET is_archived = 0 WHERE id = ?", [$restore_id]);
            
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex11_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}: {$record['telecom_provider']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex11 record #{$delete_id} without permission");
                header('Location: annex11_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex11_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}: {$record['telecom_provider']}"
                );
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 11: Archived Communication Lines Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex11_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived communication line reports.</p>
                                    <a href="annex11_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Telecom Provider,Interruption Date,Restored Date,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex11_archived_" + new Date().toISOString().split('T')[0] + ".csv");
