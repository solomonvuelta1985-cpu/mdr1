...
define('PAGE_TITLE', 'Annex 18 - Animal Evacuation Records');
...
if (!check_rate_limit($user_id, 'annex18_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex18 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex18_records_access',
    'Accessed Annex 18 animal evacuation records page'
);
...
log_security_event(
    $_SESSION['user_id'],
    'page_access',
    'Accessed Annex 18 records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex18_animal_evacuation LIKE 'is_archived'");
    ...
    if ($column_check->rowCount() == 0) {
        // SECURITY FIX - DO NOT AUTO-ALTER: // $pdo->exec("ALTER TABLE annex18_animal_evacuation ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex18 records CSRF token mismatch');
        header('Location: annex18_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex18_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex18 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex18');
            header('Location: annex18_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex18_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex18 delete rate limit exceeded');
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex18 record #{$delete_id} without permission");
                header('Location: annex18_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("DELETE FROM annex18_animal_evacuation WHERE id = ?", [$delete_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex18_delete_success',
                    "Successfully deleted Annex18 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex18 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex18');
            header('Location: annex18_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex18_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex18 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex18 record #{$archive_id} without permission");
                header('Location: annex18_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex18_animal_evacuation SET is_archived = 1 WHERE id = ?", [$archive_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex18_archive_success',
                    "Successfully archived Annex18 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex18 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
if ($is_admin) {
    ...
    $stmt = db_query("
        ...
        FROM annex18_animal_evacuation a 
    ...
    $stmt = db_query("
        SELECT * FROM annex18_animal_evacuation 
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-paw"></i> Animal Evacuation Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 18</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            ...
            <p><i class="fas fa-hashtag"></i> Total Animals</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $total_livestock; ?></h3>
            <p><i class="fas fa-horse"></i> Livestock</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $total_poultry; ?></h3>
            <p><i class="fas fa-egg"></i> Poultry</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex18.php" class="btn-custom btn-primary-custom">
                <i class="fas fa-plus"></i> Add New Record
            </a>
            <a href="annex18_archived.php" class="btn-custom btn-secondary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            <i class="fas fa-paw" style="font-size: 3rem; color: #ccc;"></i>
                            <p style="color: #6c757d; margin-top: 15px;">No animal evacuation records found.</p>
                            <a href="annex18.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                ...
                <?php else: ?>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td><?php echo number_format($record['quantity']); ?> heads</td>
                            <td><?php echo htmlspecialchars($record['evacuation_reason']); ?></td>
                            <td>
                                <?php if (!empty($record['shelter_location'])): ?>
                                    <small><?php echo htmlspecialchars(substr($record['shelter_location'], 0, 30)); ?>...</small>
                                <?php else: ?>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Animal Evacuation Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Animal Evacuation Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex18_view.php?id=${recordId}`)
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex18_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Classification,Type,Quantity,Evacuation Reason,Shelter Location,Assistance Provider,Remarks,Created By,Created At\n";
    ...
    records.forEach(record => {
        const row = [
            ...
            `"${record.type}"`,
            record.quantity,
            `"${record.evacuation_reason}"`,
            `"${(record.shelter_location || '').replace(/"/g, '""')}"`,
    ...
    link.setAttribute("download", "annex18_animal_evacuation_" + new Date().toISOString().split('T')[0] + ".csv");
