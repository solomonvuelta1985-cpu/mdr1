...
define('PAGE_TITLE', 'Annex 18 - Archived Animal Evacuation Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex18_archived_access',
    'Accessed Annex 18 archived records page'
);
// Database connection
$pdo = $pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex18 archived CSRF token mismatch');
        header('Location: annex18_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex18 record #{$restore_id} without permission");
                header('Location: annex18_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex18_animal_evacuation WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex18_animal_evacuation SET is_archived = 0 WHERE id = ?", [$restore_id]);
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex18 record #{$delete_id} without permission");
                header('Location: annex18_archived.php');
                exit;
            }
        } else {
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 18: Archived Animal Evacuation Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex18_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived animal evacuation reports.</p>
                                    <a href="annex18_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Classification,Type,Quantity,Evacuation Reason,Shelter Location,Assistance Provider,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex18_archived_" + new Date().toISOString().split('T')[0] + ".csv");
