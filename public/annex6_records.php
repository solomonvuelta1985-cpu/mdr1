...
define('PAGE_TITLE', 'Annex 6 - Infrastructure Damage Records');
...
if (!check_rate_limit($user_id, 'annex6_records_access', 100, 3600)) {
...
log_audit_action(
    $_SESSION['user_id'],
    'annex6_records_access',
    'Accessed Annex 6 infrastructure damage records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex6_infrastructure_damage LIKE 'is_archived'");
    ...
    if ($column_check->rowCount() == 0) {
        // SECURITY FIX - DO NOT AUTO-ALTER: // $pdo->exec("ALTER TABLE annex6_infrastructure_damage ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER total_damaged");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        header('Location: annex6_records.php');
        exit;
    }

    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex6_post_action', 30, 3600)) {
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!check_rate_limit($user_id, 'annex6_delete', 10, 3600)) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            ...
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }

        if ($record) {
            $stmt = db_query("DELETE FROM annex6_infrastructure_damage WHERE id = ?", [$delete_id]);
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!check_rate_limit($user_id, 'annex6_archive', 20, 3600)) {
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            ...
            $check_stmt = db_query("SELECT * FROM annex6_infrastructure_damage WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }

        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex6_infrastructure_damage SET is_archived = 1 WHERE id = ?", [$archive_id]);
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-building"></i> Infrastructure Damage Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 6</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            ...
            <p><i class="fas fa-bridge-water"></i> Bridges</p>
        </div>
        <div class="stat-card">
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex6.php" class="btn-custom btn-primary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '11' : '10'; ?>" style="text-align: center; padding: 30px;">
                            ...
                            <p style="color: #6c757d; margin-top: 15px;">No infrastructure damage records found.</p>
                            <a href="annex6.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                $badge_class = match($record['type']) {
                                    'Road' => 'badge-road',
                                    'Bridge' => 'badge-bridge',
                                    'Schools' => 'badge-school',
                                    'Government Facilities' => 'badge-gov',
                                    'Health Facilities' => 'badge-health',
                                    default => 'badge-other'
                                };
                                ?>
                            ...
                            <td>
                                <?php if ($record['classification']): ?>
                                    <span class="badge <?php echo $record['classification'] === 'National' ? 'badge-national' : 'badge-local'; ?>">
                            ...
                            <td class="text-center"><strong><?php echo number_format($record['totally_damaged']); ?></strong></td>
                            <td class="text-center"><strong><?php echo number_format($record['partially_damaged']); ?></strong></td>
                            <td class="text-center">
                                <strong style="color: #dc3545;"><?php echo number_format($record['total_damaged']); ?></strong>
                            </td>
                            <td>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Infrastructure Damage Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Infrastructure Damage Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex6_view.php?id=${recordId}`)
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex6_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Type,Classification,Infrastructure Name,Totally Damaged,Partially Damaged,Total Damaged,Unit,Quantity,Cost,Remarks,Created By,Created At\n";
    ...
    link.setAttribute("download", "annex6_infrastructure_damage_" + new Date().toISOString().split('T')[0] + ".csv");
