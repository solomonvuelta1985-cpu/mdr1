...
define('PAGE_TITLE', 'Annex 9 - Archived Power Supply Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex9_archived_access',
    'Accessed Annex 9 archived records page'
);
...
try {
    ...
    if ($column_check->rowCount() == 0) {
        // SECURITY FIX - DO NOT AUTO-ALTER: // $pdo->exec("ALTER TABLE annex9_power_supply ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex9 archived CSRF token mismatch');
        header('Location: annex9_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex9_power_supply WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex9 record #{$restore_id} without permission");
                header('Location: annex9_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex9_power_supply WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex9_power_supply SET is_archived = 0 WHERE id = ?", [$restore_id]);
            
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex9_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}: {$record['service_provider']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex9 record #{$delete_id} without permission");
                header('Location: annex9_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex9_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}: {$record['service_provider']}"
                );
...
if ($is_admin) {
    $stmt = db_query("
        SELECT p.*, u.full_name, u.barangay as user_barangay 
    ...
    $stmt = db_query("
        SELECT * FROM annex9_power_supply 
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 9: Archived Power Supply Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex9_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived power supply reports.</p>
                                    <a href="annex9_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
                    ...
                    <?php else: ?>
                        <?php foreach ($archived_records as $record): ?>
                            <tr>
                                ...
                                <td><strong><?php echo htmlspecialchars($record['service_provider']); ?></strong></td>
                                <td><?php echo date('M j, Y g:i A', strtotime($record['interruption_date'])); ?></td>
                                <td>
                                ...
                                <td>
                                    <small><?php echo htmlspecialchars(substr($record['remarks'] ?? 'N/A', 0, 50)); ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Power Supply Details</h5>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Service Provider,Interruption Date,Restored Date,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex9_archived_" + new Date().toISOString().split('T')[0] + ".csv");
