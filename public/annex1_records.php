...
define('PAGE_TITLE', 'Annex 1 - Related Incident Records');
...
if (!check_rate_limit($user_id, 'annex1_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex1 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex1_records_access',
    'Accessed Annex 1 related incident records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex1_related_incidents LIKE 'is_archived'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex1 records CSRF token mismatch');
        header('Location: annex1_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex1_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex1 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex1');
            header('Location: annex1_records.php');
            exit;
        }
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex1_related_incidents WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex1 record #{$delete_id} without permission");
                header('Location: annex1_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex1_related_incidents WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex1_delete',
                    "Permanently deleted record #{$delete_id} for {$record['barangay']}: {$record['incident_type']}"
                );
                
                // SECURITY LOG: Successful deletion
                log_security_event(
                    $_SESSION['user_id'],
                    'annex1_delete_success',
                    "Successfully deleted Annex1 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex1 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex1');
            header('Location: annex1_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex1_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex1 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex1_related_incidents WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex1 record #{$archive_id} without permission");
                header('Location: annex1_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex1_related_incidents WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex1_archive',
                    "Archived record #{$archive_id} for {$record['barangay']}: {$record['incident_type']}"
                );
                
                // SECURITY LOG: Successful archival
                log_security_event(
                    $_SESSION['user_id'],
                    'annex1_archive_success',
                    "Successfully archived Annex1 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex1 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-exclamation-triangle"></i> Related Incident Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 1</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            ...
            <p><i class="fas fa-exclamation-circle"></i> Total Incidents</p>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
            ...
            <p><i class="fas fa-calendar-alt"></i> Last Updated</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex1.php" class="btn-custom btn-primary-custom">
                <i class="fas fa-plus"></i> Add New Incident
            </a>
            <a href="annex1_archived.php" class="btn-custom btn-secondary-custom">
                <i class="fas fa-archive"></i> View Archived
            </a>
            <button onclick="exportToCSV()" class="btn-custom btn-success-custom">
                <i class="fas fa-file-csv"></i> Export CSV
            </button>
            <button onclick="openPrintPreview()" class="btn-custom btn-info-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            ...
                            <p style="color: #6c757d; margin-top: 15px;">No incident records found.</p>
                            <a href="annex1.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($record['description'])): ?>
                                    <div class="text-truncate-custom" title="<?php echo htmlspecialchars($record['description']); ?>">
                            ...
                            <td>
                                <?php if (!empty($record['actions_taken'])): ?>
                                    <div class="text-truncate-custom" title="<?php echo htmlspecialchars($record['actions_taken']); ?>">
                            ...
                            <td>
                                <?php if (!empty($record['remarks'])): ?>
                                    <div class="text-truncate-custom" title="<?php echo htmlspecialchars($record['remarks']); ?>">
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Incident Details</h5>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex1_view.php?id=${recordId}`)
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Incident Type,Occurrence Date,Description,Actions Taken,Remarks,Created By,Created At\n";
    ...
    link.setAttribute("download", "annex1_incident_records_" + new Date().toISOString().split('T')[0] + ".csv");
