...
define('PAGE_TITLE', 'Annex 5 - Agriculture Damage Records');
...
if (!check_rate_limit($user_id, 'annex5_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex5 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex5_records_access',
    'Accessed Annex 5 agriculture damage records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex5_agriculture_damage LIKE 'is_archived'");
    if ($column_check->rowCount() == 0) {
        $pdo->exec("ALTER TABLE annex5_agriculture_damage ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER total_damaged");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex5 records CSRF token mismatch');
        header('Location: annex5_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex5_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex5 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex5');
            header('Location: annex5_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex5_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex5 delete rate limit exceeded');
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex5 record #{$delete_id} without permission");
                header('Location: annex5_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("DELETE FROM annex5_agriculture_damage WHERE id = ?", [$delete_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex5_delete',
                    "Permanently deleted record #{$delete_id} for {$record['barangay']}: {$record['classification']} - {$record['type']}"
                );
                
                // SECURITY LOG: Successful deletion
                log_security_event(
                    $_SESSION['user_id'],
                    'annex5_delete_success',
                    "Successfully deleted Annex5 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex5 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex5');
            header('Location: annex5_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex5_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex5 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex5 record #{$archive_id} without permission");
                header('Location: annex5_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex5_agriculture_damage SET is_archived = 1 WHERE id = ?", [$archive_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex5_archive',
                    "Archived record #{$archive_id} for {$record['barangay']}: {$record['classification']} - {$record['type']}"
                );
                
                // SECURITY LOG: Successful archival
                log_security_event(
                    $_SESSION['user_id'],
                    'annex5_archive_success',
                    "Successfully archived Annex5 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex5 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
foreach ($records as $record) {
    $total_damage_value += $record['damage_value'] ?? 0;
    $total_affected_people += $record['affected_people'] ?? 0;
    
    switch ($record['classification']) {
        case 'Crops':
            $total_crops++;
            break;
        case 'Livestock and Poultry':
            $total_livestock++;
            break;
        case 'Fisheries':
            $total_fisheries++;
            break;
        case 'Agricultural Infrastructure':
            $total_infrastructure++;
            break;
        case 'Machineries and Equipment':
            $total_machinery++;
            break;
    }
}
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-seedling"></i> Agriculture Damage Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 5</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            <h3><?php echo number_format($total_affected_people); ?></h3>
            <p><i class="fas fa-users"></i> Affected People</p>
        </div>
        <div class="stat-card">
            <h3>
                <?php 
                if ($total_damage_value >= 1000000) {
                    echo '₱' . number_format($total_damage_value / 1000000, 1) . 'M';
                } else if ($total_damage_value >= 1000) {
                    echo '₱' . number_format($total_damage_value / 1000, 1) . 'K';
                } else {
                    echo '₱' . number_format($total_damage_value, 0);
                }
                ?>
            </h3>
            <p><i class="fas fa-money-bill-wave"></i> Total Damage</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $total_crops; ?></h3>
            <p><i class="fas fa-wheat-alt"></i> Crops</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $total_livestock; ?></h3>
            <p><i class="fas fa-cow"></i> Livestock</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $total_fisheries; ?></h3>
            <p><i class="fas fa-fish"></i> Fisheries</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex5.php" class="btn-custom btn-primary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            <i class="fas fa-seedling" style="font-size: 3rem; color: #ccc;"></i>
                            <p style="color: #6c757d; margin-top: 15px;">No agriculture damage records found.</p>
                            <a href="annex5.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                $badge_class = match($record['classification']) {
                                    'Crops' => 'badge-crops',
                                    'Livestock and Poultry' => 'badge-livestock',
                                    'Fisheries' => 'badge-fisheries',
                                    'Agricultural Infrastructure' => 'badge-infrastructure',
                                    'Machineries and Equipment' => 'badge-machinery',
                            ...
                            <td><?php echo number_format($record['affected_people']); ?></td>
                            <td><strong>₱<?php echo number_format($record['damage_value'], 2); ?></strong></td>
                            <td>
                                <?php if ($record['classification'] === 'Crops'): ?>
                                    <small>Area: <?php echo number_format($record['total_crop_area'], 2); ?> ha</small>
                                    <br><small>Loss: <?php echo number_format($record['production_loss_volume'], 2); ?> MT</small>
                                <?php elseif ($record['classification'] === 'Livestock and Poultry'): ?>
                                    <small>Heads: <?php echo number_format($record['animal_heads']); ?></small>
                                <?php elseif ($record['classification'] === 'Fisheries'): ?>
                                    <small><?php echo htmlspecialchars(substr($record['fisheries_details'] ?? 'N/A', 0, 30)); ?>...</small>
                                <?php else: ?>
                                    <small>Total: <?php echo number_format($record['total_damaged']); ?> units</small>
                                <?php endif; ?>
                            </td>
                            <td>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Agriculture Damage Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Agriculture Damage Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex5_view.php?id=${recordId}`)
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex5_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Classification,Type,Affected People,Damage Value,No Recovery Area,With Recovery Area,Total Crop Area,Production Loss Volume,Animal Heads,Fisheries Details,Totally Damaged,Partially Damaged,Total Damaged,Created By,Created At\n";
    ...
    records.forEach(record => {
        const row = [
            ...
            `"${(record.fisheries_details || '').replace(/"/g, '""')}"`,
    ...
    link.setAttribute("download", "annex5_agriculture_damage_" + new Date().toISOString().split('T')[0] + ".csv");
