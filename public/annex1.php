...
define('PAGE_TITLE', 'Annex 1 - Related Incident');
...
log_audit_action($_SESSION['user_id'], 'annex1_form_access', 'User accessed the Annex 1 Related Incident form');
...
function validate_incident_entry($entry) {
    $errors = [];
    
    // Required fields
    $required = ['region', 'province', 'city', 'barangay', 'incident_type'];
    ...
    if (!empty($entry['occurrence_date']) && !strtotime($entry['occurrence_date'])) {
        $errors[] = "Invalid occurrence date format";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'])) {
        ...
        header('Location: annex1.php');
        exit;
    }
    
    // Rate limiting - check if user has submitted too many requests
    if (!check_rate_limit($_SESSION['user_id'], 'annex1_submission', 10, 300)) { // 10 submissions per 5 minutes
    ...
    if (isset($_POST['region']) && is_array($_POST['region'])) {
        foreach ($_POST['region'] as $index => $region) {
            ...
            if (empty($region) && 
                ...
                empty($_POST['incident_type'][$index])) {
                continue;
            }
            ...
            $entry_data = [
                ...
                'incident_type' => sanitize($_POST['incident_type'][$index] ?? ''),
                'occurrence_date' => sanitize($_POST['occurrence_date'][$index] ?? ''),
                'description' => sanitize($_POST['description'][$index] ?? ''),
                'actions_taken' => sanitize($_POST['actions_taken'][$index] ?? ''),
                'remarks' => sanitize($_POST['remarks'][$index] ?? '')
            ];
            
            // Validate entry
            $entry_errors = validate_incident_entry($entry_data);
            ...
            try {
                ...
                $result = $stmt->execute([
                    ...
                    $entry_data['city'], 
                    $entry_data['barangay'],
                    $entry_data['incident_type'], 
                    $entry_data['occurrence_date'], 
                    $entry_data['description'],
                    $entry_data['actions_taken'], 
                    $entry_data['remarks'], 
                    $_SESSION['user_id']
                ]);
                ...
                if ($result) {
                    ...
                    log_audit_action($_SESSION['user_id'], 'annex1_submission', 
                        "Submitted incident report for " . $entry_data['barangay']);
                } else {
                    $error_count++;
                }
            } catch (PDOException $e) {
                // Log error but don't expose details to user
                error_log("Annex1 database error for user {$_SESSION['user_id']}: " . $e->getMessage());
                $error_count++;
            }
        }
    }
    ...
    // Set flash messages
    if ($success_count > 0) {
        set_flash("Successfully saved $success_count incident report(s).", 'success');
    }
    if ($error_count > 0) {
...
<body>
    ...
    <div class="form-container">
        ...
        <div id="formContent" class="form-content">
            ...
            <form id="relatedIncidentForm" method="POST">
                ...
                <div id="entriesContainer">
                    ...
                    <div class="entry-card lazy-entry loaded">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6 col-lg-6">
                                <label for="incident_type-0" class="form-label">Type of Incident</label>
                                <select class="form-select" id="incident_type-0" name="incident_type[]" required>
                            ...
                            <div class="col-md-6 col-lg-6">
                                <label for="occurrence_date-0" class="form-label">Date/Time of Occurrence</label>
                                <input type="datetime-local" class="form-control" id="occurrence_date-0" name="occurrence_date[]">
                            </div>
                        </div>

                        <div class="section-title mt-4">Response Details</div>
                        <div class="row g-3">
                            <div class="col-12">
                                <label for="description-0" class="form-label">Description</label>
                                <textarea class="form-control" id="description-0" name="description[]" placeholder="Provide description of the incident" maxlength="1000"></textarea>
                            </div>
                            <div class="col-12">
                                <label for="actions_taken-0" class="form-label">Actions Taken</label>
                                <textarea class="form-control" id="actions_taken-0" name="actions_taken[]" placeholder="Describe actions taken" maxlength="1000"></textarea>
                            </div>
                            <div class="col-12">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <strong>General Guidelines:</strong> The table for related incidents is dedicated for reporting secondary incidents or hazards caused by the primary incidents or hazards being monitored. A fire incident following an earthquake or flooding resulting from a weather disturbance fall into this category. The primary hazard event or incident should not be included in this table.
                    </div>
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <p><strong>Region, Province, City/Municipality, Barangay</strong><br>
                            Indicate the specific location where the related incident occurred.</p>
                            <p><strong>Type of Incident</strong><br>
                            Categorize the incident using standard terms such as landslide, flooding, fire, earthquake, etc. This allows for quick grouping and identification of similar incidents.</p>
                            <p><strong>Date and Time</strong><br>
                            Indicate the exact date and time the incident occurred or was observed.</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Description</strong><br>
                            Provide a brief summary of the incident.</p>
                            <div class="alert alert-light">
                                <em>Example: "The landslide incident affected four residential houses."</em>
                            </div>
                            <p><strong>Actions Taken</strong><br>
                            Specify the response actions initiated by the concerned agencies or LGUs.</p>
                            <div class="alert alert-light">
                                <em>Example: "The LDRRMO deployed emergency response teams to the affected area."</em>
                            </div>
                            <p><strong>Remarks</strong><br>
                            Include any other relevant details or contextual information about the incident.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('addEntryBtn').addEventListener('click', function() {
            ...
            setTimeout(function() {
                ...
                newEntry.innerHTML = `
                    ...
                    <div class="row g-3">
                        <div class="col-md-6 col-lg-6">
                            <label for="incident_type-${entryCount}" class="form-label">Type of Incident</label>
                            <select class="form-select" id="incident_type-${entryCount}" name="incident_type[]" required>
                                <option value="">Select Type</option>
                                <option value="Landslide">Landslide</option>
                                <option value="Flooding">Flooding</option>
                                <option value="Fire">Fire</option>
                        ...
                        <div class="col-md-6 col-lg-6">
                            <label for="occurrence_date-${entryCount}" class="form-label">Date/Time of Occurrence</label>
                            <input type="datetime-local" class="form-control" id="occurrence_date-${entryCount}" name="occurrence_date[]">
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <label for="description-${entryCount}" class="form-label">Description</label>
                            <textarea class="form-control" id="description-${entryCount}" name="description[]" placeholder="Provide description of the incident" maxlength="1000"></textarea>
                        </div>
                        <div class="col-12">
                            <label for="actions_taken-${entryCount}" class="form-label">Actions Taken</label>
                            <textarea class="form-control" id="actions_taken-${entryCount}" name="actions_taken[]" placeholder="Describe actions taken" maxlength="1000"></textarea>
                        </div>
                        <div class="col-12">
        ...
        document.getElementById('relatedIncidentForm').addEventListener('submit', function(e) {
        ...
        document.getElementById('relatedIncidentForm').addEventListener('submit', function(e) {
            ...
            entries.forEach((entry, index) => {
                ...
                const incidentType = entry.querySelector('select[name="incident_type[]"]');
                
                // Skip validation if all fields are empty
                if (!region.value && !province.value && !city.value && !barangay.value && !incidentType.value) {
                    return;
                }
                ...
                const description = entry.querySelector('textarea[name="description[]"]');
                const actionsTaken = entry.querySelector('textarea[name="actions_taken[]"]');
                const remarks = entry.querySelector('textarea[name="remarks[]"]');
                
                if (description.value.length > 1000) {
                    isValid = false;
                    alert(`Description exceeds maximum length of 1000 characters in entry ${index + 1}`);
                    return;
                }
                
                if (actionsTaken.value.length > 1000) {
                    isValid = false;
                    alert(`Actions taken exceeds maximum length of 1000 characters in entry ${index + 1}`);
                    return;
                }
                
                if (remarks.value.length > 500) {
                    isValid = false;
                    alert(`Remarks exceeds maximum length of 500 characters in entry ${index + 1}`);
                    return;
                }
            });
