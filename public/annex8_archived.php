...
define('PAGE_TITLE', 'Annex 8 - Archived Records');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex8 archived CSRF token mismatch');
        header('Location: annex8_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex8_road_bridge_status WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex8 record #{$restore_id} without permission");
                header('Location: annex8_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex8_road_bridge_status WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex8_road_bridge_status SET is_archived = 0 WHERE id = ?", [$restore_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex8_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}: {$record['type']} - {$record['road_section']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex8 record #{$delete_id} without permission");
                header('Location: annex8_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex8_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}: {$record['type']} - {$record['road_section']}"
                );
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 8: Archived Road and Bridge Status Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex8_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived road and bridge status reports.</p>
                                    <a href="annex8_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    // Headers
    csvContent += "ID,Barangay,Type,Classification,Road/Bridge Name,Status,Date Not Passable,Date Passable,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex8_archived_records_" + new Date().toISOString().split('T')[0] + ".csv");
