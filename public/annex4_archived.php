...
define('PAGE_TITLE', 'Annex 4 - Archived Records');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        header('Location: annex4_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT id FROM annex4_damaged_houses WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            if (!$check_stmt->fetch()) {
        ...
        $stmt = db_query("UPDATE annex4_damaged_houses SET is_archived = 0 WHERE id = ?", [$restore_id]);
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT id FROM annex4_damaged_houses WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            if (!$check_stmt->fetch()) {
                set_flash('Record not found or access denied', 'error');
                header('Location: annex4_archived.php');
                exit;
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 4: Archived Damaged Houses Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex4_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived damaged house reports.</p>
                                    <a href="annex4_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    // Headers
    csvContent += "ID,Barangay,Totally Damaged,Partially Damaged,Total Damaged,Cost,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex4_archived_records_" + new Date().toISOString().split('T')[0] + ".csv");
