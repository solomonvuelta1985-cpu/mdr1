...
define('PAGE_TITLE', 'Annex 21 - Assistance to LGUs Records');
...
if (!check_rate_limit($user_id, 'annex21_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex21 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex21_records_access',
    'Accessed Annex 21 assistance to LGUs records page'
);
...
log_security_event(
    $_SESSION['user_id'],
    'page_access',
    'Accessed Annex 21 records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex21_assistance_lgu LIKE 'is_archived'");
    if ($column_check->rowCount() == 0) {
        $pdo->exec("ALTER TABLE annex21_assistance_lgu ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex21 records CSRF token mismatch');
        header('Location: annex21_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex21_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex21 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex21');
            header('Location: annex21_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex21_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex21 delete rate limit exceeded');
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ? AND created_by = ?", [$delete_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex21 record #{$delete_id} without permission");
                header('Location: annex21_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ?", [$delete_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("DELETE FROM annex21_assistance_lgu WHERE id = ?", [$delete_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex21_delete',
                    "Permanently deleted record #{$delete_id} for {$record['recipient']}: {$record['cluster']} - {$record['type']}"
                );
                
                // SECURITY LOG: Successful deletion
                log_security_event(
                    $_SESSION['user_id'],
                    'annex21_delete_success',
                    "Successfully deleted Annex21 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex21 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex21');
            header('Location: annex21_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex21_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex21 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex21 record #{$archive_id} without permission");
                header('Location: annex21_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex21_assistance_lgu SET is_archived = 1 WHERE id = ?", [$archive_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex21_archive',
                    "Archived record #{$archive_id} for {$record['recipient']}: {$record['cluster']} - {$record['type']}"
                );
                
                // SECURITY LOG: Successful archival
                log_security_event(
                    $_SESSION['user_id'],
                    'annex21_archive_success',
                    "Successfully archived Annex21 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex21 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
if ($is_admin) {
    ...
    $stmt = db_query("
        SELECT * FROM annex21_assistance_lgu 
...
<style>
    /* Copy the same CSS styles from annex5_records.php and adjust colors as needed */
    body {
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-hand-holding-heart"></i> Assistance to LGUs Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 21</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards">
        ...
        <div class="stat-card">
            <h3><?php echo $is_admin ? 'All' : htmlspecialchars($user_location['barangay']); ?></h3>
            <p><i class="fas fa-map-marker-alt"></i> Barangay</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex21.php" class="btn-custom btn-primary-custom">
                <i class="fas fa-plus"></i> Add New Record
            </a>
            <a href="annex21_archived.php" class="btn-custom btn-secondary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '11' : '10'; ?>" style="text-align: center; padding: 30px;">
                            <i class="fas fa-hand-holding-heart" style="font-size: 3rem; color: #ccc;"></i>
                            <p style="color: #6c757d; margin-top: 15px;">No assistance to LGUs records found.</p>
                            <a href="annex21.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                ...
                <?php else: ?>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td>
                                <strong><?php echo htmlspecialchars($record['recipient']); ?></strong>
                                <br><small class="text-muted"><?php echo htmlspecialchars($record['barangay'] ?? $record['user_barangay'] ?? 'N/A'); ?></small>
                            ...
                            <td>
                                ...
                                $badge_class = match($record['cluster']) {
                                    ...
                                    'Camp Management' => 'badge-camp',
                                    'Emergency Telecommunications' => 'badge-telecom',
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Assistance to LGUs Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Assistance to LGUs Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function viewRecord(recordId) {
    ...
    fetch(`../api/annex21_view.php?id=${recordId}`)
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex21_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Recipient,Cluster,Type,Quantity,Unit,Cost per Unit,Amount,Source,Remarks,Created By,Created At\n";
    ...
    link.setAttribute("download", "annex21_assistance_lgu_" + new Date().toISOString().split('T')[0] + ".csv");
