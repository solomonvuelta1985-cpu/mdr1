...
if (!check_rate_limit($user_id, 'annex8_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex8 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex8_records_access',
    'Accessed Annex 8 road and bridge status records page'
);
...
try {
    $column_check = $pdo->query("SHOW COLUMNS FROM annex8_road_bridge_status LIKE 'is_archived'");
    if ($column_check->rowCount() == 0) {
        $pdo->exec("ALTER TABLE annex8_road_bridge_status ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER remarks");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!check_rate_limit($user_id, 'annex8_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex8 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex8');
            header('Location: annex8_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex8_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex8 delete rate limit exceeded');
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex8_delete_success',
                    "Successfully deleted Annex8 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex8 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex8');
            header('Location: annex8_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex8_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex8 archive rate limit exceeded');
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_security_event(
                    $_SESSION['user_id'],
                    'annex8_archive_success',
                    "Successfully archived Annex8 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex8 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-road"></i> Road and Bridge Status Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 8</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            ...
            <p><i class="fas fa-bridge"></i> Bridges</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $passable_count; ?></h3>
            <p><i class="fas fa-check-circle"></i> Passable</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $not_passable_count; ?></h3>
            <p><i class="fas fa-times-circle"></i> Not Passable</p>
        </div>
        <div class="stat-card">
            <h3><?php echo $heavy_only_count + $light_only_count; ?></h3>
            <p><i class="fas fa-exclamation-circle"></i> Restricted</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex8.php" class="btn-custom btn-primary-custom">
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            <i class="fas fa-road" style="font-size: 3rem; color: #ccc;"></i>
                            <p style="color: #6c757d; margin-top: 15px;">No road and bridge status records found.</p>
                            <a href="annex8.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                ...
                <?php else: ?>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                $classification_class = match(explode('/', $record['classification'])[0]) {
                                    'National' => 'badge-national',
                                    'Provincial' => 'badge-provincial',
                                    'City' => 'badge-city',
                                    'Barangay' => 'badge-barangay',
                                    default => 'badge-secondary'
                                };
                                ?>
                                <span class="badge <?php echo $classification_class; ?>">
                                    <?php echo htmlspecialchars($record['classification']); ?>
                                </span>
                            </td>
                            <td><strong><?php echo htmlspecialchars($record['road_section']); ?></strong></td>
                            <td>
                                <?php
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="viewRecordModalLabel">Road and Bridge Status Details</h5>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Road and Bridge Status Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Type,Classification,Road/Bridge Name,Status,Date Not Passable,Date Passable,Remarks,Image Path,Created By,Created At\n";
