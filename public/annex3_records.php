...
define('PAGE_TITLE', 'Annex 3 - Casualties Records');
...
if (!check_rate_limit($user_id, 'annex3_records_access', 100, 3600)) {
    log_security_event($user_id, 'rate_limit_exceeded', 'Annex3 records page access rate limit exceeded');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex3_records_access',
    'Accessed Annex 3 casualties records page'
);
...
try {
    ...
    if ($column_check->rowCount() == 0) {
        // SECURITY FIX - DO NOT AUTO-ALTER: // $pdo->exec("ALTER TABLE annex3_casualties ADD COLUMN is_archived TINYINT(1) DEFAULT 0 AFTER validated");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex3 records CSRF token mismatch');
        header('Location: annex3_records.php');
        exit;
    }
    
    // RATE LIMITING: Check action-specific rate limits
    if (!check_rate_limit($user_id, 'annex3_post_action', 30, 3600)) {
        log_security_event($user_id, 'rate_limit_exceeded', 'Annex3 POST action rate limit exceeded');
    ...
    if (isset($_POST['delete_id'])) {
        ...
        if (!$delete_id) {
            ...
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid delete_id for Annex3');
            header('Location: annex3_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for delete operations
        if (!check_rate_limit($user_id, 'annex3_delete', 10, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex3 delete rate limit exceeded');
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                ...
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to delete Annex3 record #{$delete_id} without permission");
                header('Location: annex3_records.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex3_delete',
                    "Permanently deleted record #{$delete_id} for {$record['barangay']}: {$record['surname']}, {$record['first_name']}"
                );
                
                // SECURITY LOG: Successful deletion
                log_security_event(
                    $_SESSION['user_id'],
                    'annex3_delete_success',
                    "Successfully deleted Annex3 record #{$delete_id}"
                );
                
                set_flash('Record deleted successfully', 'success');
            } else {
                set_flash('Failed to delete record', 'error');
                log_security_event($_SESSION['user_id'], 'delete_failure', "Failed to delete Annex3 record #{$delete_id}");
            }
        } else {
    ...
    if (isset($_POST['archive_id'])) {
        ...
        if (!$archive_id) {
            set_flash('Invalid record ID', 'error');
            log_security_event($_SESSION['user_id'], 'invalid_input', 'Invalid archive_id for Annex3');
            header('Location: annex3_records.php');
            exit;
        }
        
        // RATE LIMITING: Specific check for archive operations
        if (!check_rate_limit($user_id, 'annex3_archive', 20, 3600)) {
            log_security_event($user_id, 'rate_limit_exceeded', 'Annex3 archive rate limit exceeded');
        ...
        // Check if user owns the record (unless admin)
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex3_casualties WHERE id = ? AND created_by = ?", [$archive_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to archive Annex3 record #{$archive_id} without permission");
                header('Location: annex3_records.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex3_casualties WHERE id = ?", [$archive_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            // Archive the record
            $stmt = db_query("UPDATE annex3_casualties SET is_archived = 1 WHERE id = ?", [$archive_id]);
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex3_archive',
                    "Archived record #{$archive_id} for {$record['barangay']}: {$record['surname']}, {$record['first_name']}"
                );
                
                // SECURITY LOG: Successful archival
                log_security_event(
                    $_SESSION['user_id'],
                    'annex3_archive_success',
                    "Successfully archived Annex3 record #{$archive_id}"
                );
                
                set_flash('Record archived successfully', 'success');
            } else {
                set_flash('Failed to archive record', 'error');
                log_security_event($_SESSION['user_id'], 'archive_failure', "Failed to archive Annex3 record #{$archive_id}");
            }
        } else {
            set_flash('Record not found', 'error');
        }
...
foreach ($records as $record) {
    ...
    switch ($record['sex']) {
        case 'Male': $total_male++; break;
        case 'Female': $total_female++; break;
    }
}
...
<style>
    ...
    .stat-card:nth-child(6) { border-left-color: #198754; }
    .action-section {
        display: flex;
        flex-wrap: wrap;
...
<div class="records-container">
    ...
    <div class="header-section">
        <h1><i class="fas fa-user-injured"></i> Casualties Records</h1>
        <h2>NDRRMC Memorandum Circular No. 05, s. 2025 - Annex 3</h2>
        <?php if ($is_admin): ?>
    ...
    <div class="stats-cards" id="statsCards">
        ...
        <div class="stat-card">
            <h3><?php echo number_format($total_dead); ?></h3>
            <p><i class="fas fa-skull"></i> Deceased</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_injured); ?></h3>
            <p><i class="fas fa-user-injured"></i> Injured</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_ill); ?></h3>
            <p><i class="fas fa-stethoscope"></i> Ill</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_missing); ?></h3>
            <p><i class="fas fa-search"></i> Missing</p>
        </div>
        <div class="stat-card">
            <h3><?php echo number_format($total_validated); ?></h3>
            <p><i class="fas fa-check-circle"></i> Validated</p>
        </div>
    </div>
    ...
    <div class="action-section">
        ...
        <div class="action-buttons">
            <a href="annex3.php" class="btn-custom btn-primary-custom">
                <i class="fas fa-plus"></i> Add New Casualty
            </a>
                ...
                <?php if (empty($records)): ?>
                    <tr>
                        <td colspan="<?php echo $is_admin ? '10' : '9'; ?>" style="text-align: center; padding: 30px;">
                            <i class="fas fa-user-injured" style="font-size: 3rem; color: #ccc;"></i>
                            <p style="color: #6c757d; margin-top: 15px;">No casualties records found.</p>
                            <a href="annex3.php" class="btn-custom btn-primary-custom" style="margin-top: 15px;">
                ...
                <?php else: ?>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td><strong><?php echo htmlspecialchars($record['barangay'] ?? $record['user_barangay'] ?? 'N/A'); ?></strong></td>
...
<div class="modal fade" id="editRecordModal" tabindex="-1" aria-labelledby="editRecordModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRecordModalLabel">Edit Casualty Record</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
function editRecord(recordId) {
    ...
    fetch(`../api/annex3_edit.php?id=${recordId}`)
        .then(response => response.text())
        .then(data => {
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Category,Surname,First Name,Middle Name,Age,Sex,Address,Cause,Remarks,Source,Validated,Created At\n";
    ...
    link.setAttribute("download", "annex3_casualties_" + new Date().toISOString().split('T')[0] + ".csv");
