...
define('PAGE_TITLE', 'Annex 5 - Archived Agriculture Damage Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex5_archived_access',
    'Accessed Annex 5 archived records page'
);
// Database connection
$pdo = $pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex5 archived CSRF token mismatch');
        header('Location: annex5_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex5 record #{$restore_id} without permission");
                header('Location: annex5_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex5_agriculture_damage WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex5_agriculture_damage SET is_archived = 0 WHERE id = ?", [$restore_id]);
            
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex5_restore',
                    "Restored record #{$restore_id} for {$record['barangay']}: {$record['classification']} - {$record['type']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex5 record #{$delete_id} without permission");
                header('Location: annex5_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex5_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['barangay']}: {$record['classification']} - {$record['type']}"
                );
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 5: Archived Agriculture Damage Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex5_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived agriculture damage reports.</p>
                                    <a href="annex5_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Classification,Type,Affected People,Damage Value,No Recovery Area,With Recovery Area,Total Crop Area,Production Loss Volume,Animal Heads,Fisheries Details,Totally Damaged,Partially Damaged,Total Damaged,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex5_archived_" + new Date().toISOString().split('T')[0] + ".csv");
