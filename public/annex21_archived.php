...
define('PAGE_TITLE', 'Annex 21 - Archived Assistance to LGUs Records');
...
log_audit_action(
    $_SESSION['user_id'],
    'annex21_archived_access',
    'Accessed Annex 21 archived records page'
);
// Database connection
$pdo = $pdo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_token($_POST['csrf_token'] ?? '')) {
        ...
        log_security_event($_SESSION['user_id'], 'csrf_failure', 'Annex21 archived CSRF token mismatch');
        header('Location: annex21_archived.php');
        exit;
    }
    ...
    if (isset($_POST['restore_id'])) {
        ...
        if (!$is_admin) {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ? AND created_by = ?", [$restore_id, $_SESSION['user_id']]);
            $record = $check_stmt->fetch();
            
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to restore Annex21 record #{$restore_id} without permission");
                header('Location: annex21_archived.php');
                exit;
            }
        } else {
            $check_stmt = db_query("SELECT * FROM annex21_assistance_lgu WHERE id = ?", [$restore_id]);
            $record = $check_stmt->fetch();
        }
        
        if ($record) {
            $stmt = db_query("UPDATE annex21_assistance_lgu SET is_archived = 0 WHERE id = ?", [$restore_id]);
            
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex21_restore',
                    "Restored record #{$restore_id} for {$record['recipient']}: {$record['cluster']} - {$record['type']}"
                );
    ...
    if (isset($_POST['permanent_delete_id'])) {
        ...
        if (!$is_admin) {
            ...
            if (!$record) {
                set_flash('Record not found or access denied', 'error');
                log_security_event($_SESSION['user_id'], 'unauthorized_access', "Attempted to permanently delete Annex21 record #{$delete_id} without permission");
                header('Location: annex21_archived.php');
                exit;
            }
        } else {
        ...
        if ($record) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                log_audit_action(
                    $_SESSION['user_id'],
                    'annex21_permanent_delete',
                    "Permanently deleted archived record #{$delete_id} for {$record['recipient']}: {$record['cluster']} - {$record['type']}"
                );
...
<style>
    /* Copy the same CSS styles from annex5_archived.php and adjust colors as needed */
    body {
...
<div class="container-fluid">
    <div class="records-container">
        <div class="header-section">
            ...
            <h2>Annex 21: Archived Assistance to LGUs Records</h2>
            <?php if ($is_admin): ?>
        ...
        <div class="d-flex justify-content-between mb-4 flex-wrap gap-3">
            <a href="annex21_records.php" class="btn btn-primary">
                        ...
                        <tr>
                            <td colspan="9">
                                <div class="empty-state">
                                    ...
                                    <p>There are no archived assistance to LGUs reports.</p>
                                    <a href="annex21_records.php" class="btn btn-primary mt-3">
                                        <i class="fas fa-arrow-left me-2"></i>Back to Active Records
                                    </a>
                                </div>
                            </td>
                        ...
                        <?php foreach ($archived_records as $record): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <br><small class="text-muted"><?php echo htmlspecialchars($record['barangay'] ?? $record['user_barangay'] ?? 'Unknown'); ?></small>
...
function exportToCSV() {
    ...
    csvContent += "ID,Region,Province,City,Barangay,Recipient,Cluster,Type,Quantity,Unit,Cost per Unit,Amount,Source,Remarks,Created By,Archived Date\n";
    ...
    link.setAttribute("download", "annex21_archived_" + new Date().toISOString().split('T')[0] + ".csv");
